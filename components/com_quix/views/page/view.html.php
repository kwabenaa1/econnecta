...
class QuixViewPage extends JViewLegacy
{
     ...
     * @throws Exception
     */
    protected function _checkAmp()
    {
        $amp_default = $this->config->get('amp_default', true);
        $metadata = new Registry;
        $metadata->loadString($this->item->metadata);
        $enable_amp = $metadata->get('enable_amp', '');
        if ($enable_amp == 'false') {
            return;
        }elseif($enable_amp == '' && !$amp_default){
            return;
        }
